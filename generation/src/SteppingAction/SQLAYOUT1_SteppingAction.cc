...
int SteppingAction::WhichXYbin(double xpos, double ypos, int zbin){
  ...
  for (int i=1; i<=nbinsx[zbin]; i++){
    ...
    //if ((xpos < -25 + i*50/nbinsx[zbin]) && (xpos > -25)){
    if ((xpos < -500 + i*1000/nbinsx[zbin]) && (xpos > -500)){
      xbin = i - 1;
      break;
    }
  }
  for (int i=1; i<=nbinsy[zbin]; i++){
    ...
    //if ((ypos < -25 + i*50/nbinsy[zbin]) && (ypos > -25)){
    if ((ypos < -500 + i*1000/nbinsy[zbin]) && (ypos > -500)){
      ybin = i - 1;
      break;
    }
  }
